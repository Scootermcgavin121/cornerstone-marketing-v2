...
### QuickBooks / AP Hardening (NEW — same day)
The QuickBooks Online integration got a serious accounts-payable upgrade today. Highlights (full detail in the QuickBooks Online Integration section below):
- **Proper two-stage accounting flow:** a sent PO posts to QuickBooks as a **Purchase Order (commitment)**; when the work is received/invoiced, the matching **Bill** auto-creates and **closes the PO** — no dangling POs, no duplicates. Payment still stays your call (manual approval in QuickBooks; Cornerstone never auto-pays).
- **Smart due-date scheduling for batch pay,** now including a **semi-monthly** cadence (two paydays a month). Due dates key off when the vendor is actually invoiced, so payables land on the days you cut checks.
- **Full sync visibility:** an **Accounting → Sync Events** log shows every push to QuickBooks and lets you retry anything.
- **Clean memos** on every synced transaction (community, lot, address, PO#, cost code) so your accountant knows exactly what each line is.
- **Automatic sales tax** on materials (labor exempt) flows through to QuickBooks on POs and change orders.
...
- **Cornerstone syncs to QuickBooks Online — automatically. Live and shipped today; data flows one direction (Cornerstone → QuickBooks), plus one inbound signal: paid status.** Connect once and the Purchase Orders, Change Orders, Bills, and Vendors you approve in Cornerstone post themselves to QuickBooks. No manual entry, no CSV exports, no double-keying. The only thing that flows back is read-only payment status — when you pay a Bill in QuickBooks, the linked PO marks itself Paid (details below). This is **one-way sync PLUS inbound payment status — NOT two-way/bidirectional** sync.
...
- **Proper two-stage accounting flow — commitment, then bill.** When you send a PO, Cornerstone posts a matching **Purchase Order** to QuickBooks as a *commitment* (money you've promised but don't owe yet). When the work is received/invoiced, Cornerstone auto-creates the matching **Bill** and **closes the PO against it** — so you get the full accrual lifecycle in QuickBooks with **no dangling PO and no duplicate**. This is real construction AP, not just dumping bills into the books.
...
- **AP payment terms & automatic due dates — batch-pay on one payday.** Set how each bill comes due and Cornerstone computes the due date for you off **the date the vendor is actually invoiced/billed**, then carries it into QuickBooks so you can pay everything in one run. Because the due date keys off the real invoice date, your payables land on the days you actually cut checks. Supported schedules:
  - **Net-X days** — due X days after the bill date (Net-15, Net-30, Net-45, whatever you set).
  - **Weekly** — due on a chosen weekday every week.
  - **Semi-monthly** — two paydays a month (e.g. the 1st and 15th, or the 15th and the last day), so bills fall onto whichever payday comes next.
...
- **Full sync visibility — an Accounting → Sync Events log.** Every push to QuickBooks is recorded in a **Sync Events** log under Accounting, so you can see exactly what synced, when, and whether it succeeded — and **retry anything** that needs it. No black box: you always know the state of your books.
- **Clean, accountant-ready memos on every transaction.** Each synced PO, Bill, or change order carries a clear memo with the **community, lot, address, PO number, and cost code** — so anyone opening QuickBooks knows exactly what every line is without hunting back through Cornerstone.
- **Inbound paid status — pay in QuickBooks and the PO marks itself Paid.** When you pay a vendor Bill in QuickBooks, that paid status flows back and Cornerstone **automatically marks the linked purchase order Paid**, stamped with the **QuickBooks payment date and amount**. Partial payments are handled: the PO only flips to Paid once the QuickBooks bill balance reaches **$0**. This is the one exception to the one-way flow, and it is deliberately narrow — it's **one-way sync PLUS an inbound payment-status signal**, not two-way/bidirectional sync. Cornerstone only **reads** the payment result to reflect it on the PO; it **never pushes edits or money back into QuickBooks, and never auto-pays** a vendor. Payment always happens manually in QuickBooks — this simply mirrors the resulting paid status so your PO lifecycle closes out without re-keying.
...
- **The end-to-end AP loop:** a construction task completes → the matching PO auto-generates and emails the vendor → the PO syncs to QuickBooks → (optionally) the Bill auto-creates in QuickBooks on completion, linked to the PO → the builder batch-pays everything on payday using the computed due dates → the paid status flows back and the linked PO marks itself **Paid** (with the QuickBooks payment date and amount). The vendor's only job is to send an invoice. Every dollar rolls up per home (each Home a QuickBooks Project) for true profit-per-house.
...
- **Granular included-vs-upgrade control at the individual-option level** - builders aren't limited to including/excluding whole categories. To turn a currently-included Level 1 (Standard) option into a paid upgrade, open it from Purchasing → Options and change its Spec Level to a higher tier (Upgrade I, Upgrade II, or Premium); on save it drops out of the base price and becomes a +$X upgrade. To exclude an entire option class from the base budget instead, use the Standard/Upgrade toggle on the Spec Levels page. Builders control what's included down to the single option, not just whole categories.
...
- Pricing uses **base price + attribute modifiers** from vendor bids — no manual 7,000-row price matrix needed
...
- **Customer (buyer) record limit:** Each organization's active customer record limit is based on its plan: **Starter and Builder up to 500**, **Pro up to 1,000**, **Pro+ up to 3,000**, and **Enterprise up to 5,000**. Organizations approaching their plan limit (at 90%) receive in-app warnings. Once the limit is reached (100%), new customers can't be added until existing records are deleted. Need more capacity? Contact us about an Enterprise plan or higher custom limits.
...
A: Yes - each organization's active customer (buyer) record limit is based on its plan: Starter and Builder include up to 500, Pro up to 1,000, Pro+ up to 3,000, and Enterprise up to 5,000. You'll get an in-app warning as you approach your limit (at 90%), and once you reach 100% you'll need to delete existing records before adding new ones. If you need more capacity, contact us about our Enterprise plan or higher custom limits.

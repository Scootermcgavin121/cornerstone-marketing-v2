...
const comparisonRows: ComparisonRow[] = [
  ...
  { feature: "Automatic displaced-standard credit PO on design-center upgrades", cornerstone: true, competitor: false, note: "Buildertrend automates PO generation but not substitution. When a buyer upgrades, Buildertrend generates the upgrade PO automatically — but the credit for the displaced standard is a separate manual variance PO a human has to remember to create, on every swap, on every lot. Cornerstone models the displacement in the option itself, so the upgrade PO AND the base credit fire from the same selection event, routed to the right vendors, with zero human deciding whether the credit happened. At production volume that's the difference between purchasing that scales and hundreds of hand-created credit POs." },
  ...
  { feature: "Pricing model", cornerstone: "Flat — up to 30 users (60 on Pro+)", competitor: "Custom quote", note: "Buildertrend no longer publishes pricing -- custom quotes only. Cornerstone is flat at every tier — up to 30 users (60 on Pro+). More importantly: Buildertrend doesn't ship a structural options engine, floorplan pricing matrix, multi-community pipelines, native AI agents, or 37+ named webhook events at any price point." },
...
const faqItems: FAQItem[] = [
  ...
  {
    q: "How does Cornerstone handle the PO credit when a buyer upgrades a design-center selection?",
    a: "Automatically — and this is a real gap against Buildertrend. Buildertrend automates PO generation (bulk wizard, from estimates/bids/selections), so the upgrade PO fires fine. What it doesn't automate is the credit for the standard the buyer walked away from: that's a separate manual variance PO someone has to remember to create on every swap, on every lot. Cornerstone models the substitution in the option itself — one selection event fires the upgrade PO and the displaced-standard credit together, both routed to the right vendors, with a built-in audit trail. The work is done once at catalog setup, reused across every lot, instead of a manual change order on every design-center upgrade. At production volume (40 lots × a dozen upgrades each) that's the difference between purchasing that scales and hundreds of hand-created credit POs — each one a chance to over-charge the buyer or under-credit the trade.",
  },
  {

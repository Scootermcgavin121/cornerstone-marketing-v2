...
const purchasingSchema = buildFeatureSchema({
  ...
  featureList: [
    ...
    "Task-completion PO generation (draft-first) — auto-generate a draft PO when a construction task is marked complete, pre-filled via cost codes, reviewed before sending, downloadable as PDF",
...
const features = [
  ...
  { title: "Flexible PO Generation — Four Patterns", desc: "Most platforms hard-code one PO model. Cornerstone gives you four: (1) all POs at job start (lock in budget exposure day one), (2) partial deposit POs from any earlier task (permit pull, contract sign, foundation pour), (3) draft PO auto-generated when a construction task is marked complete (cash goes out only when work is done — reviewed before sending), or (4) configurable per task / scope / community — mix and match on the same job. Your cash flow rules drive the system, not the other way around." },
  { title: "Task-Completion PO Generation (Draft-First)", desc: "Never forget to order materials. The moment you mark a construction task complete, Cornerstone auto-generates the associated purchase order &mdash; as a draft, so the PM reviews before anything reaches the vendor. Each draft is pre-filled with the correct parts + scope items (via cost codes) at the right quantities for that home's floorplan, plus vendor and pricing &mdash; zero manual data entry. The mapping is explicit: Construction Task &rarr; Cost Code &rarr; Takeoff Items &rarr; Draft PO. Download any PO as a PDF to email the vendor yourself. No residential platform auto-generates POs from task completion at this level of detail; BuilderTrend and CoConstruct require fully manual PO creation." },

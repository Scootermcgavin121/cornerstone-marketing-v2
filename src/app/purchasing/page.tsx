...
const purchasingSchema = buildFeatureSchema({
  ...
  featureList: [
    ...
    "QuickBooks Online sync (live) — one-way push of POs, Bills, sales tax, vendors, and job costs into QuickBooks, with construction-grade job costing per home and community (each Community a QuickBooks Customer, each Home a Project)",
    "Optional auto-Bill on PO completion — mark a PO received and Cornerstone creates the matching Bill in QuickBooks, linked to the PO, with payment left as a manual approval (never auto-paid)",
    "AP payment terms & automatic due dates — Net-X days, weekly, monthly (Nth or last weekday), or bi-weekly anchored to a payday; the computed due date carries into QuickBooks so builders batch-pay on one payday",
    "Inbound paid status — pay a vendor Bill in QuickBooks and Cornerstone marks the linked PO Paid automatically, with the QuickBooks payment date and amount (partial payments handled; PO flips to Paid only when the bill balance hits $0). Read-only status reflection, not two-way sync",
...
const features = [
  ...
  { title: "QuickBooks Auto-Bill on Completion", desc: "Optional per-toggle: mark a purchase order received/complete and Cornerstone creates the matching Bill in QuickBooks, linked to the PO so it closes out with no duplicate. Payment always stays a manual approval in QuickBooks — Cornerstone never auto-pays a vendor. The human pay gate is yours." },
  { title: "AP Payment Terms & Batch-Pay", desc: "Set how each bill comes due — Net-X days, weekly on any weekday, monthly on the Nth (or last) weekday, or bi-weekly anchored to a payday you pick. Cornerstone computes the due date automatically and carries it into QuickBooks, so you can batch-pay every vendor on a single payday instead of chasing invoices one at a time." },
  { title: "QuickBooks Paid Status Flows Back", desc: "When you pay a vendor Bill in QuickBooks, that paid status flows back and Cornerstone automatically marks the linked purchase order Paid — stamped with the QuickBooks payment date and amount, so your PO lifecycle closes out without re-keying anything. Partial payments are handled: the PO only flips to Paid once the QuickBooks bill balance reaches $0. This is one-way sync plus a single inbound status signal — a read-only reflection of what happened in QuickBooks, not two-way sync. Cornerstone never pushes edits or money into QuickBooks, and never auto-pays a vendor." },
      ...
      <section id="change-orders" className="py-16 px-4 scroll-mt-24">
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="p-8 rounded-2xl bg-slate-900/60 border border-slate-800">
            ...
            <h2 className="text-2xl font-black text-white mb-3">QuickBooks Online sync <span className="text-emerald-400 text-base align-middle">(live)</span></h2>
            <p className="text-slate-400 leading-relaxed mb-5">
